...
func NewMsgUbtDepositedClaim(creator string, eventNonce uint64, blockHeight uint64, tokenContract string, amount sdk.Int, ethereumSender string, baseledgerAddress string, ubtPrice string) *MsgUbtDepositedClaim {
	return &MsgUbtDepositedClaim{
		...
		BaseledgerReceiverAccountAddress: baseledgerAddress,
		UbtPrice:                         ubtPrice,
	}
}
...
func (msg *MsgUbtDepositedClaim) ClaimHash() ([]byte, error) {
	path := fmt.Sprintf("%d/%d/%s/%s/%s/%s", msg.EventNonce, msg.BlockHeight, msg.TokenContract, msg.Amount, msg.EthereumSender, msg.BaseledgerReceiverAccountAddress)
